...
export interface TrialDemoState {
  job: { id: string; title: string; company: string; owner: string };
  candidate: { id: string; name: string; matchScore: number; strengths: string[]; gaps: string[] };
  application: { id: string; stage: string; reportIncluded: boolean; resubmissionAllowed: boolean };
  trial_task: {
    id: string;
  ...
  ai_run: { status: AiRunStatus; activeStep: number; failNext: boolean; error?: string };
}
...
export const PRESET_TASK = {
  ...
  background: "公司计划为校招平台增加 AI 简历分析能力，需要在两周内完成首版验证。",
  requirements: [
    "明确目标用户和核心问题",
    "设计 MVP 功能范围",
    "描述 AI 与普通程序的职责边界",
...
export const PRESET_SUBMISSION = {
  body: "首版只处理候选人主动上传的简历。规则程序负责文件类型、字段完整性和确定性校验，AI 负责归纳经历、解释岗位差距并给出待确认建议。所有 AI 结果都需要用户确认后才能写入画像，系统不自动抓取外部链接，也不根据敏感属性推断候选人能力。核心流程为上传简历、结构化提取、差距分析、人工确认和生成报告。首版成功指标建议使用完成一次分析并确认结果的用户占比，同时观察错误字段的人工修正率。隐私方面仅保存完成服务所需的数据，提供删除入口；错误结果需要展示来源证据、置信提示和人工修正能力。",
  prototypeUrl: "https://example.com/hirelink-demo",
  attachments: [{ name: "AI简历分析MVP方案.pdf", size: 2_480_000, type: "application/pdf" }],
...
export const PRESET_EVALUATION: NonNullable<TrialDemoState["trial_evaluation"]> = {
  ...
  dimensions: [
    ...
    },
    {
      ...
      score: 25,
      achieved: 22,
      evidence: "覆盖上传、提取、分析、确认和报告生成主流程。",
    },
    {
      id: "scope",
      label: "MVP 范围控制",
      score: 20,
      achieved: 18,
      evidence: "明确不做自动录用、复杂推荐和跨平台数据同步。",
    },
    {
      id: "risk",
      label: "AI 风险意识",
      score: 20,
      achieved: 17,
      evidence: "提出敏感属性禁区、来源证据、置信提示和人工修正能力。",
    },
    {
      id: "clarity",
      label: "表达清晰度",
      score: 10,
      achieved: 8,
      evidence: "方案结构清楚，但指标统计窗口和目标阈值仍可补充。",
    },
  ],
  highlight: "能够区分规则程序与生成式 AI 的职责。",
  gap: "成功指标定义仍偏宽泛。",
  risk: "需要进一步明确错误字段修正率的基线、统计周期和可接受阈值。",
...
function createInitialState(): TrialDemoState {
  return {
    ...
    application: {
      id: "application_ai_pm_li_001",
      stage: "待生成岗位能力试炼",
      reportIncluded: false,
      resubmissionAllowed: false,
    },
    trial_task: { ...PRESET_TASK, status: "ungenerated", source: "ai" },
    trial_submission: {
      status: "empty",
      body: "",
      prototypeUrl: "",
...
let activeApplicationId = state.application.id;
let hydrated = false;
...
export const trialDemoService = {
  async load(applicationId: string = activeApplicationId) {
    activeApplicationId = applicationId;
    try {
      const next = await hirelinkApi.trialState(applicationId);
      if (next) {
        state = next;
        hydrated = true;
        emit();
  ...
  usePresetTask() {
    ...
    update((current) => ({
      ...
      trial_task: { ...PRESET_TASK, status: "draft", source: "fallback", generatedAt: nowIso() },
    }));
  },
  updateTask(
    task: Pick<
  ...
  saveDraft() {
    ...
    update((current) => ({
      ...
      trial_submission: {
        ...
        saveError: shouldFail ? "草稿保存失败，请重试。" : undefined,
        saveFailNext: false,
      },
    }));
  ...
  failNextEvaluation() {
    update((current) => ({ ...current, ai_run: { ...current.ai_run, failNext: true } }));
  },
  runEvaluation() {
    clearTimers();
    ...
    update((current) => ({
      ...current,
      trial_evaluation: null,
      ai_run: { status: "running", activeStep: 0, failNext: false },
    }));
        ...
        update((current) =>
          shouldFail
            ...
            : {
                ...
                ai_run: { status: "completed", activeStep: AI_RUN_STEPS.length, failNext: false },
              },
        ),
      1500,
    );
  ...
  usePresetEvaluation() {
    ...
    update((current) => ({
      ...
      trial_task: { ...current.trial_task, status: "evaluated" },
      trial_evaluation: PRESET_EVALUATION,
      ai_run: { status: "fallback", activeStep: AI_RUN_STEPS.length, failNext: false },
    }));
  },
  openResubmission() {
    update((current) => ({
