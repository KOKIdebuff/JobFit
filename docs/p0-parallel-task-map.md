...
> 状态：Frozen v1（历史任务定义冻结，不代表任务已执行完成）\\
> 启动闸门：主控线程确认三份 P0 基线文档一致后，才允许启动 child agent。  
> 总原则：公共文件单一所有者；业务线程发现契约缺失时提交变更请求，不得直接修改公共基线。
> 当前代码状态：本文是早期并行任务地图，当前实现已超过 F0-F5 原始范围。认证、核心 demo API、真人面试后端、通知后端和部分前端联调已存在；实际状态以 `docs/implementation-status.md` 为准。
...
> 注意：每个任务中的 `Done When` 是历史验收口径，不是当前仓库完成状态。当前是否已实现必须回到 `docs/implementation-status.md` 和代码证据判断。
