...
> 观察快照：`main` 的代码提交 `d736b7e` 加本轮未提交的 JF-P1-01 Provider、审计 migration、测试和文档同步；证据来自路由注册、JobFit Runtime、模型、迁移、前端路径、配置和 pytest。
>
...
- Settings 以 `JOBFIT_*` 为主，并直接读取有限的 `HIRELINK_*` legacy 别名；Provider 仅由服务端读取，openai_compatible 有 URL / HTTPS 约束，前端还保留 `VITE_HIRELINK_API_BASE_URL` 兼容读取。其存在不证明旧 Runtime、导航或产品能力。
...
但不证明命名真实 LLM、外部知识库、浏览器语音识别质量、端到端浏览器旅程或生产运行质量。每次实际执行结果、环境和验证级别由 [progress.md](./progress.md) 的事件记录引用，不由本文自行声明通过；P1 阶段边界见 [p1-implementation-protocol.md](./p1-implementation-protocol.md)。
